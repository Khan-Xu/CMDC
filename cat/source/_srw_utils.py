...
class _srw_electron_beam(object):
    ...
    def after_monte_carlo(self, rand_array, period_length, 
        ...
        auxp_x0 = (
            self.mc["sigma_px"] * rand_array[1] + 
            self.mc["ax"] * auxp_xp / self.mc["gx"]
            )
        auxp_y0 = (
            self.mc["sigma_py"] * rand_array[3] + 
            self.mc["ay"] * auxp_yp / self.mc["gy"]
            )
        ...
        _part_beam.partStatMom1.gamma = (
                    ...
                    (1 + np.sqrt(self.part_beam.arStatMom2[10]) * 
                    rand_array[4])
                    )
